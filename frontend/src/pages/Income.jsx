...
export default function Income() {
  ...
  const [formData, setFormData] = useState({ amount: '', description: '', date: new Date().toISOString().split('T')[0], type: 'INCOME' });
  ...
  const handleEdit = (tx) => {
    setFormData({ amount: tx.amount, description: tx.description, date: new Date(tx.date).toISOString().split('T')[0], type: 'INCOME' });
    setEditingId(tx.id);
    setShowForm(true);
  };
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-200">
            {transactions.length === 0 && (
              <tr><td colSpan="4" className="p-4 text-center text-slate-600 font-medium">No income records found</td></tr>
            )}
            {transactions.map(tx => (
