...
export default function Expenses() {
  ...
  const [formData, setFormData] = useState({ amount: '', description: '', date: new Date().toISOString().split('T')[0], type: 'EXPENSE' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ amount: '', description: '', date: new Date().toISOString().split('T')[0], type: 'EXPENSE' });
      setEditingId(null);
      setShowForm(false);
      fetchTransactions();
    } catch (error) {
  ...
  const handleEdit = (tx) => {
    setFormData({ amount: tx.amount, description: tx.description, date: new Date(tx.date).toISOString().split('T')[0], type: 'EXPENSE' });
    setEditingId(tx.id);
    setShowForm(true);
  };
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-6">
          ...
          <button 
            ...
            className="bg-rose-600 text-white px-4 py-2 rounded-lg font-medium flex items-center gap-2 hover:bg-rose-700 transition-colors"
          >
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm mb-8">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-slate-800">{editingId ? 'Edit' : 'New'} Expense Entry</h2>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input type="number" required value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} className="w-full px-4 py-2 border rounded-lg" placeholder="e.g. 500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Purpose / Description</label>
              <input type="text" required value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full px-4 py-2 border rounded-lg" placeholder="e.g. Food, Travel" />
            </div>
            <div>
            ...
            <div className="md:col-span-3 flex justify-end">
              <button type="submit" className="bg-indigo-600 text-white px-8 py-2 rounded-lg font-medium hover:bg-indigo-700">Save Expense</button>
            </div>
          </form>
        </div>
      )}
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-200">
            {transactions.length === 0 && (
              <tr><td colSpan="4" className="p-4 text-center text-slate-600 font-medium">No expense records found</td></tr>
            )}
            {transactions.map(tx => (
